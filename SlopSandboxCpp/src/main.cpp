...
static constexpr float kGroundHalfThicknessPx = 18.0f;
...
public:
    ...
    void Run()
    {
        ...
        InitWindow(m_width, m_height, "SlopSandbox");
        SetTargetFPS(120);
        InitUIFont();
...
private:
    ...
    float MeasureTextUi(const std::string& text, float fontSize) const
    {
        if (m_uiFontLoaded)
        {
            return MeasureTextEx(m_uiFont, text.c_str(), fontSize, 1.0f).x;
        }
    ...
    void InitWorld()
    {
        ...
        worldDef.maximumLinearSpeed = 110.0f;
        ...
        shapeDef.material.friction = 1.35f;
    ...
    Vector2 ClampSpawnAboveGround(Vector2 posPx, float halfWidthPx, float halfHeightPx) const
    {
        ...
        float maxY = top - halfHeightPx - 2.0f;
    ...
    void ApplyBodySurface(size_t idx)
    {
        ...
        if (e.isSlippery)
        {
            friction = std::min(friction, 0.03f);
            rolling = 0.0f;
        }
        if (e.isSticky)
        {
            friction = std::max(friction, 2.8f);
            rolling = std::max(rolling, 0.08f);
        }
        if (e.isBouncy)
        {
        ...
        if (cap <= 0) return;
        std::vector<b2ShapeId> shapeIds(static_cast<size_t>(cap));
        int count = b2Body_GetShapes(e.bodyId, shapeIds.data(), cap);
        for (int i = 0; i < count; ++i)
        {
            b2SurfaceMaterial mat = b2Shape_GetSurfaceMaterial(shapeIds[i]);
            mat.friction = friction;
            mat.restitution = restitution;
            mat.rollingResistance = rolling;
            b2Shape_SetSurfaceMaterial(shapeIds[i], &mat);
        }
    }
    ...
    void SpawnBox(Vector2 pos)
    {
        ...
        shapeDef.material.restitution = 0.01f;
    ...
    void SpawnCircle(Vector2 pos)
    {
        ...
        b2ShapeDef shapeDef = b2DefaultShapeDef();
        shapeDef.density = 1.0f;
        shapeDef.material.friction = 0.72f;
        shapeDef.material.restitution = 0.01f;
    ...
    {
        // Base width and height are both exactly kBaseSizePx.
        float h = kBaseSizePx;
        float halfBase = h * 0.5f;
    ...
    void SpawnPolygonBody(Vector2 centerPx, const std::vector<Vector2>& localVertices)
    {
        ...
        shapeDef.material.friction = 0.9f;
        shapeDef.material.restitution = 0.0f;
        shapeDef.material.rollingResistance = 0.03f;
    ...
    std::optional<size_t> PickBody(Vector2 mousePx)
    {
        ...
        for (size_t i = m_bodies.size(); i > 0; --i)
        {
            ...
            if (cap <= 0) continue;
            std::vector<b2ShapeId> shapeIds(static_cast<size_t>(cap));
            int count = b2Body_GetShapes(m_bodies[idx].bodyId, shapeIds.data(), cap);
            for (int s = 0; s < count; ++s)
            {
                if (b2Shape_TestPoint(shapeIds[s], p)) return idx;
            }
    ...
    void UpdateGlass(float dt)
    {
        ...
        for (size_t i = 0; i < m_bodies.size(); ++i)
        {
            ...
            if (cap > 0)
            {
                std::vector<b2ContactData> contacts(static_cast<size_t>(cap));
                int count = b2Body_GetContactData(e.bodyId, contacts.data(), cap);
                float impulse = 0.0f;
                for (int c = 0; c < count; ++c)
                {
                    const b2Manifold& m = contacts[c].manifold;
                    for (int p = 0; p < 2; ++p)
                    {
            ...
            // load contribution from weight above
            Vector2 center = ToPixels(b2Body_GetPosition(e.bodyId));
            float load = 0.0f;
            int cap2 = b2Body_GetContactCapacity(e.bodyId);
            if (cap2 > 0)
            {
                std::vector<b2ContactData> contacts(static_cast<size_t>(cap2));
                int count2 = b2Body_GetContactData(e.bodyId, contacts.data(), cap2);
                for (int c = 0; c < count2; ++c)
                {
                    b2BodyId a = b2Shape_GetBody(contacts[c].shapeIdA);
                    b2BodyId b = b2Shape_GetBody(contacts[c].shapeIdB);
    ...
    void HandleMouse()
    {
        ...
        {
            if ((IsMouseButtonPressed(MOUSE_BUTTON_LEFT) || (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && !m_draggingBodies && !m_selecting)) && !m_drawing)
            {
                StartBodyDrag(mouse);
            }
            if (IsMouseButtonDown(MOUSE_BUTTON_LEFT))
    ...
    void UpdateSimulation(float dt)
    {
        ...
        if (b2Body_IsValid(m_groundBody))
        {
            ...
            b2Vec2 targetP = ToMeters({m_width * 0.5f, ActiveGroundCenterYPx()});
            float dx = std::abs(gt.p.x - targetP.x);
            float dy = std::abs(gt.p.y - targetP.y);
            if (dx > 0.0001f || dy > 0.0001f || std::abs(gt.q.s) > 0.0001f)
            {
                gt.p = targetP;
                gt.q = b2MakeRot(0.0f);
    ...
    void DrawBody(const BodyEntry& b)
    {
        ...
        world.reserve(b.localVertsPx.size());
        float cs = std::cos(angle);
        float sn = std::sin(angle);
        for (const Vector2& lv : b.localVertsPx)
        {
            world.push_back({c.x + lv.x * cs - lv.y * sn, c.y + lv.x * sn + lv.y * cs});
        }

        for (size_t i = 1; i + 1 < world.size(); ++i)
        {
            DrawTriangle(world[0], world[i], world[i + 1], fill);
        }

        for (size_t i = 0; i < world.size(); ++i)
        {
            size_t j = (i + 1) % world.size();
            DrawLineEx(world[i], world[j], 2.2f, stroke);
        }
        ...
        if (b.selected)
        {
            ...
            {
                size_t j = (i + 1) % world.size();
                DrawLineEx(world[i], world[j], 5.0f, Color{80, 170, 255, 120});
            }
        }
    ...
    void DrawWater()
    {
        ...
        pts.reserve(m_waveDisp.size() + 2);
        for (size_t i = 0; i < m_waveDisp.size(); ++i)
        {
            float x = static_cast<float>(i) * m_waveStep;
            pts.push_back({x, m_waveBaselineY + m_waveDisp[i]});
        }
    ...
    void DrawDrawPreview()
    {
        ...
        {
            Rectangle r = NormalizeRect(m_drawStart, m_drawCurrent);
            Vector2 a{r.x + r.width * 0.5f, r.y};
            Vector2 b{r.x + r.width, r.y + r.height};
            Vector2 d{r.x, r.y + r.height};
